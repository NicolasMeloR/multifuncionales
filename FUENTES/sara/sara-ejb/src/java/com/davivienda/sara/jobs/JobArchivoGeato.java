...
@Stateless(mappedName = "JobArchivoGeato")
@Remote({ JobArchivoGeatoRemote.class })
@RemoteHome(JobArchivoGeatoHome.class)
public class JobArchivoGeato extends BaseServicio implements JobArchivoGeatoRemote, TimedObject
{
    @Resource
    ...
    public void ejbTimeout(Timer timer) {
        try {
            (this.loggerApp = SaraConfig.obtenerInstancia().loggerApp).info(System.getProperty("weblogic.Name") + " com.davivienda.sara.jobs.JobArchivoGeato.ejbTimeout()");
            this.cargarObjetos();
            String respuesta = "";
            ...
            try {
                ...
                if (this.ConsultarEstadoCargue().equals("0")) {
                    ...
                    intRegProcesadosTotal = this.procesarArchivoGeato(fechaInicial, fechaConsulta);
                    respuesta = "Se Actualizaron con exito en la bd : " + intRegProcesadosTotal + " Registros de Totales Egresos ";
                    this.em.flush();
                }
                final TimerService timerService = this.context.getTimerService();
                ...
                System.out.println(System.getProperty("weblogic.Name") + " EJB Timed Out ... prox eje: " + timer.getNextTimeout());
            }
            ...
            this.loggerApp.info(System.getProperty("weblogic.Name") + " com.davivienda.sara.jobs.JobArchivoGeato.ejbTimeout():" + respuesta);
        }
        catch (Exception exc) {
            exc.printStackTrace();
        }
    ...
    private int procesarArchivoGeato(final Calendar fechaInicial, final String fechaConsulta) {
        int intRegProcesadosTotal = -1;
        final String strArchivoCarga = "geato002";
        ...
        try {
            ...
            if (archivoCargado == null) {
                ...
                intRegProcesadosTotal = this.cargarArchivoTotalesEgresos(fechaInicial, cargueautomatico);
                this.actualizarRegCargueArchivo(strArchivoCarga, cargueautomatico, fechaInicial, "", intRegProcesadosTotal);
            }
            else {
            ...
            try {
                this.loggerApp.log(Level.SEVERE, "ProcesarArchivoGeato Exception:", ex);
                ...
                this.loggerApp.log(Level.SEVERE, "ProcesarArchivoGeato EntityServicioExcepcion:", ese);
            }
        }
        return intRegProcesadosTotal;
    }
    
    public Integer cargarArchivoTotalesEgresos(final Calendar fecha, final boolean cargueautomatico) {
        this.loggerApp.log(Level.INFO, "Cargar archivo totales egresos fecha {0}", fecha.getTime());
        Integer registros = -1;
        String msgError = "";
        try {
